...
#define PAYLOAD_TX             51   /*bytes to send. 51 is the max suggested to avoid the 64 bytes packet restriction*/
#define packet_retx            10   /*bytes to send. 51 is the max suggested to avoid the 64 bytes packet restriction*/
...
#endif 
/*******************************************************************/
void LoRa_spread_factor_sweep(unsigned int payload_size, unsigned int payload_amount){
  ...
  lora.setSpreadFactor((_spreading_factor_t)i,LoRa_BW ,(_physical_type_t)LoRa_FREQ_standard);
    for (unsigned int j = 0; j <payload_amount; ++j) {
     lora.transferPacket(buffer_binary, payload_size,Tx_and_ACK_RX_timeout); 
      ...
      #endif
      delay(100);/*Convert the value in seconds to miliseconds*/
  }
   delay(1000);/*Convert the value in seconds to miliseconds*/
  }
}
 ...
 *-------------------------------------------------------*/
void setup(void){
//  digitalWrite(LED_PWR, HIGH); //Turn off Arduino Led.
...
void loop(void){
  lora.transferPacketWithConfirmed(buffer_binary,PAYLOAD_TX,SF7,SF12);
  //LoRa_spread_factor_sweep(PAYLOAD_TX,packet_retx );
  delay((unsigned int)(10*1000));/*Tx_delay_s Convert the value in seconds to miliseconds*/
}
