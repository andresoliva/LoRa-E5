...
#define LoRa_SF                  SF12     /*DR5=5.2kbps //data rate. see at https://www.thethingsnetwork.org/docs/lorawan/regional-parameters/    */
...
#define Tx_delay_s               900.5     /*delay between transmitions expressed in seconds*/
...
#define PAYLOAD_TX_222          222  /*bytes to send. */
...
void printTimeReceptiondifference(unsigned int time_tx1,unsigned int time_tx2){
   char_temp[0]='\0';
   sprintf(char_temp, "\r\nEstimated transmission time of messages with %i and %i bytes as payload: %.1f ms, %.1f ms.",PAYLOAD_TX_11,PAYLOAD_TX_51,
   lora.getTransmissionTime(PAYLOAD_TX_11),  lora.getTransmissionTime(PAYLOAD_TX_51));
   Serial.print(char_temp);//to print the obtained characters
   char_temp[0]='\0';
   sprintf(char_temp, "\r\nCalculated time difference between transmitting %i and %i bytes as payload: %.1f ms.",PAYLOAD_TX_11,PAYLOAD_TX_51,
   lora.getTransmissionTime(PAYLOAD_TX_51) -lora.getTransmissionTime(PAYLOAD_TX_11));
   Serial.print(char_temp);//to print the obtained characters
   char_temp[0]='\0';
   sprintf(char_temp, "\r\nMeasured difference between transmitting with %i and %i bytes as payload: %.1f ms.",PAYLOAD_TX_11,PAYLOAD_TX_51,
   time_tx2-time_tx1);
   Serial.print(char_temp);//to print the obtained characters
}
#endif
...
void loop(void)
{
  ...
  time_tx1=lora.transferPacketWithConfirmed(buffer_binary,PAYLOAD_TX_11,Tx_and_ACK_RX_timeout);
  //time_tx2=lora.transferPacketWithConfirmed(buffer_binary,PAYLOAD_TX_51,Tx_and_ACK_RX_timeout);
  //time_tx2=lora.transferPacketWithConfirmed(buffer_binary,PAYLOAD_TX_51,Tx_and_ACK_RX_timeout);
