...
```
## Gateway Configuration to allow 
Take into mind I am using a . So the field configuration can be different to the one used in the SeeD examples/
### Enter the gateway
First. In my case, the gateway is located in a Wifi Network 
This can be different based in you Gateway set up. If you are using a 
 * Enter the web sit
In my case, I have a Milesight UG67 Gateway working on the EU868 Band. This gateway is generating a 
 * Connect your PC to the WI-FI network generated by the Gateway (Gateway_F)
 * Tipe The web address (192.168.1.1) and login using the admin and password of the gateway.
![plot](./pictures/01_enter_gateway.png)
#### Create an App Code
#### Create an App Code
#### Create an App Code
#### Wrap all together
